...
class TestPdfalyzer:
    def test_struct_elem_parent(self, analyzing_malicious_documents_pdfalyzer):
        struct_elem_node = analyzing_malicious_documents_pdfalyzer.find_node_by_idnum(120)
        assert struct_elem_node.parent.idnum == 119

    def test_all_nodes_in_tree(self, analyzing_malicious_documents_pdfalyzer, analyzing_malicious_documents_pdf_path):
        for object_id in PdfParserManager(analyzing_malicious_documents_pdf_path).object_ids:
            ...
            node = analyzing_malicious_documents_pdfalyzer.find_node_by_idnum(object_id)
